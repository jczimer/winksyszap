...
app.get("/getAllGroups", async (req, res, next) => {
    var ret = Sessions.getAllGroups(req.query.sessionName).then((ret) => {
        res.status(200).json(ret);
    }).catch (error => {
        res.status(200).json({ result: "error", message: error });
    });
...
app.get("/status", async (req, res, next) => {
    ...
    if (session) {
        ...
        res.status(200).json({ result: 200, message: "Session not found" });
    }
