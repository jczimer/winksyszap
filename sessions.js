...
module.exports = class Sessions {
    ...
    static async getAllGroups(sessionName) {
        ...
        if (session) {
            var groups = false;
            groups = await session.client.then(async (client) => {
                 return await client.getAllChatsGroups().then(async (t) => {                                       
                    return t;
                });                
            });
            console.log(groups);
            return { result: "success", groups: groups };
        ...
        if (session) {
            if (session.state == "CONNECTED") {
                ...
                await session.client.then(async client => {
                    ...
                    try {
                        ok = true;
                        return await client.sendText(params.number, params.text);
                    } catch (error) {
                        console.error();
                        ok = false;
                        message = error;                        
